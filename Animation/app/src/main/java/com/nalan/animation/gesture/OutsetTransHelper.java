...
public class OutsetTransHelper implements ViewTransHelper.Callback {
    ...
    private Matrix mMatrix,mTempMatrix;
    ...
    public void setup(int left,int top,int right,int bottom){
        ...
        mShapeWidth = mViewport.width();
        mShapeHeight = mViewport.height();

        mMatrix.reset();
        mMatrix.postTranslate(mViewport.left,mViewport.top);
        mCurrentShape.set(mViewport);
    }

    /*
        设置shape的大小，在setup以后调用它
     */
    public void setCurrentShape(Rect dst,int maxWidth,int maxHeight){
        mShapeWidth = dst.width();
        mShapeHeight = dst.height();
